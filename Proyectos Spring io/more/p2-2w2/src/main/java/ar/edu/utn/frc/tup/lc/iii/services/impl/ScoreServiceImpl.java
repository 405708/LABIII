...
public class ScoreServiceImpl implements ScoreService {
    ...
    public void calculateScore(GamePrediction gamePrediction, GameResult gameResult) {
        // TODO(Completado) 2 - Cálculo de puntajes: Esta parte del flujo calcula para cada predicción su puntaje.
        ...
        Optional<ScoreEntity> scoreEntityOptional = scoreRepository.findByUserId(gamePrediction.getUser().getId());

        Optional<ScoreDetailEntity> optionalScoreDetailEntity = null;
        if (scoreEntityOptional.isPresent()) {
            optionalScoreDetailEntity = scoreDetailRepository.findByScoreIdAndPredictionGameId(scoreEntityOptional.get().getId(), gamePrediction.getGame().getId());
            if (optionalScoreDetailEntity.isPresent()) {
                scoreEntityOptional.get().setPoints(scoreEntityOptional.get().getPoints() - optionalScoreDetailEntity.get().getScore().getPoints() + points);
                scoreRepository.save(scoreEntityOptional.get());
            }
            else
            {
                ScoreDetailEntity scoreDetailEntity = new ScoreDetailEntity();
                scoreDetailEntity.setScore(scoreEntityOptional.get());
        ...
        else
        {
            ...
            ScoreDetailEntity scoreDetailEntity = new ScoreDetailEntity();
            scoreDetailEntity.setScore(scoreRepository.save(scoreEntity));
